...
## Compile-Time Architecture Analyzer

`ChengYuan.ArchitectureAnalyzer` is not a standalone verification project. It is wired into checked projects from `Directory.Build.targets` as a compiler analyzer:
...
`OutputItemType="Analyzer"` makes normal `dotnet build` and `dotnet test` execute architecture diagnostics. `ReferenceOutputAssembly="false"` keeps the analyzer out of runtime references, so application and framework projects do not depend on analyzer assemblies at runtime.
...
`ChengYuan.Core` is a single project that owns foundational platform concerns. The old `ChengYuan.Domain` baseline is no longer part of the active architecture, and Core should not be split into multiple projects because that increases cognitive load and reference ambiguity.

The rule is simple:

- `Core` owns foundational platform concepts in one physical project.
- Internal Core areas are expressed through directories and namespaces, not separate projects or packages.
...
|---|---|---|
| `Primitives` / root `ChengYuan.Core` | Base exceptions, error codes, `Result`, DDD primitives, strongly typed IDs, object extension contracts, `IClock`, `IGuidGenerator` | EF Core, tenant context, current user context, caching, outbox |
| `Runtime` / `ChengYuan.Core.Modularity` / `ChengYuan.Core.Lifecycle` | Module descriptors, module catalog, lifecycle hooks, module bootstrap and ordering, `ServiceConfigurationContext`, init logging, async service registration, lifecycle contributors, DI primitives | Domain primitives, serializer providers, data providers |
| `Json` / `ChengYuan.Core.Json` | Serializer abstractions, System.Text.Json integration, strongly typed ID converters | EF Core, repository or UoW logic |
| `Validation` / `ChengYuan.Core.Validation` | Validation contracts and default validation pipeline | Localization resources, persistence providers |
| `Localization` / `ChengYuan.Core.Localization` | Resource registration and exception or error localization seams | Validation runtime policy, persistence providers |
| `Data` / `ChengYuan.Core.Data` | Repository contracts, unit of work contracts, data filters, persistence-facing traits such as soft delete | EF Core-specific implementation details |
...
4. Support areas next: Json, Validation, and Localization become explicit internal Core areas instead of being scattered into runtime or host code.
...
|---|---|---|
| `ChengYuan.Hosting` module primitives | `ChengYuan.Core` runtime/modularity namespace | `Hosting` stops owning modularity and becomes a thin composition helper only |
...
| `ChengYuan.Domain` Json converters | `ChengYuan.Core.Json` namespace | Serializer support becomes an explicit Core internal area, not part of the core runtime |
| `ChengYuan.Domain.EntityFrameworkCore` converters | `ChengYuan.EntityFrameworkCore` provider module | EF Core support depends on Core; Core never depends on EF Core |
...
1. Wave A: keep one `ChengYuan.Core` project and move modularity ownership out of `ChengYuan.Hosting`.
...
3. Wave C: organize Json, Validation, Localization, and Data as Core internal areas, then enforce their dependency boundaries with `ChengYuan.ArchitectureAnalyzer`.
4. Wave D: keep EF Core conventions, converters, repositories, unit of work, and data-filter implementations in `ChengYuan.EntityFrameworkCore`.
5. Wave E: rebase `ExecutionContext`, `MultiTenancy`, `Caching`, `Outbox`, and later `Authorization`, `Settings`, `Features`, and `Auditing` on the Core foundation.
...
- `Core` must not reference any other `ChengYuan.*` assembly.
- `Hosting` must not own module base types, module descriptors, or lifecycle contracts.
- Json support may live inside the single Core project; EF Core support must remain an opt-in provider dependency outside Core.
...
- `ChengYuan.Core` may not depend on any other `ChengYuan.*` assembly.
