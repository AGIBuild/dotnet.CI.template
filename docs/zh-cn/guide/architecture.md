...
仓库目前仍处于从旧的 CI 模板结构向承渊设计迁移的过程中。如果当前实现与本文档在迁移期间存在差异，后续新增开发应优先遵循本文档。

## 设计原则

- 先模块，后分层。
- 采用 ABP 的核心心智模型：将可复用切面拆成 `Framework` modules 和 `Application` modules。
...
拓扑词只属于目录，不属于项目名。优先使用 `ChengYuan.Identity.Application`，而不是 `ChengYuan.Applications.Identity.Application`。
...
`ChengYuan.Core` 是唯一允许承载基础模块化、失败模型、DDD 原语以及共享扩展缝隙的 framework module。`ChengYuan.Hosting` 在迁移完成后只能保留薄的组合辅助角色，不能继续拥有模块模型本身。
...
下一个关键架构动作，是用显式的 `ChengYuan.Core` 家族替换当前过渡态的 `ChengYuan.Domain` 基础层，并吸收 ABP Core 中真正高价值的做法。

这不是逐行复制 ABP，而是遵循更清晰的规则：

- `Core` 只拥有平台级基础概念。
- Provider 相关能力放在 Core 邻接模块中。
- `ExecutionContext`、`MultiTenancy`、`Caching`、`Outbox` 这类技术系统建立在 `Core` 之上，而不是被折叠进 `Core`。
- 在 Core 重构完成前，不再向 `ChengYuan.Domain` 继续追加新的基础能力。
...
7. Wave G：在全部引用迁移完成后，正式移除过渡态 `ChengYuan.Domain` 命名。
...
- 第一轮 Core 重构不复制 ABP 的大包数量、动态代理、Auto API 或 UI 框架层。
...
4. 目录结构固定为 `family -> module -> project`。
5. 项目名中不要带拓扑词。
...
- 在继续扩展更多 framework 或 management modules 之前，先完成 `ChengYuan.Core` 的所有权切分，并停止扩张过渡态的 `ChengYuan.Domain`。
