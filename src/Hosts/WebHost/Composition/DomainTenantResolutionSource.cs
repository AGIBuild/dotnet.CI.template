...
internal sealed class DomainTenantResolutionSource(MultiTenancyOptions options) : ITenantResolutionSource
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        foreach (var pattern in options.DomainPatterns)
        {
            ...
            // Non-Guid value — store as name candidate
    ...
    /// <summary>
    /// Converts a domain pattern like "{0}.myapp.com" into a regex and extracts the tenant segment.
    /// </summary>
    private static string? ExtractTenantFromHost(string host, string pattern)
    {
        // Escape the pattern for regex, then replace the escaped placeholder with a capture group
